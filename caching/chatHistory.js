...
export class ChatHistory {
  ...
  async setUserContext(sessionId, context) {
    ...
    const contextData = {
      ...existing,
      role: context?.role || existing.role || null,
      academicLevel: context?.academicLevel || existing.academicLevel || null,
      interests: Array.isArray(context?.interests)
        ? [...new Set([...(existing.interests || []), ...context.interests])]
        : existing.interests || [],
      intentHints: Array.isArray(context?.intentHints)
        ? [...new Set([...(existing.intentHints || []), ...context.intentHints])]
        : existing.intentHints || [],
  ...
  async updateUserContext(sessionId, partialContext) {
    if (!sessionId || !partialContext) return;
    await this.setUserContext(sessionId, partialContext);
  }

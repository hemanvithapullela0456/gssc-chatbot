...
//     }
    ...
    buildFocusedPrompt(question, context, history, language, userContext = null, conversationSummary = null) {
        const languageConfig = {
            ...
            `,
                contextInstruction:
                    'Conversation Summary में पुरानी बातचीत के मुख्य बिंदु हैं और Recent Conversation में हाल की बातचीत है। दोनों का उपयोग करके संदर्भ समझें।',
            ...
            `,
                contextInstruction:
                    'The Conversation Summary contains key facts from earlier conversation (long-term memory) and Recent Conversation shows recent exchanges (short-term memory). Use BOTH to understand full context.',
        ...
        const buildUserContextSection = (ctx) => {
            ...
            if (ctx.academicLevel) parts.push(`Academic Level: ${ctx.academicLevel}`);
            if (Array.isArray(ctx.intentHints) && ctx.intentHints.length > 0) {
                parts.push(`User intents: ${ctx.intentHints.join(', ')}`);
            }
            if (Array.isArray(ctx.interests) && ctx.interests.length > 0) {
...
${config.fallback}`;
    ...
    async chatStream(
        ...
        try {
            ...
            // Step 6: Process long-short term conversation memory.
            let processedHistory = history;
            let summaryText = conversationSummary;

            if (this.summarizer && this.summarizer.needsSummarization(history)) {
                const summaryResult = await this.summarizer.processHistory(
                    history,
                    language,
                    conversationSummary
                );

                processedHistory = summaryResult.recent;
                summaryText = summaryResult.summary;

                if (summaryResult.shouldUpdate && summaryText && chatHistoryManager) {
                    try {
                        ...
                        console.warn('[Summarizer] Failed to save summary:', err?.message || err);
                    }
                }
            }
            ...
            // Step 8: Build focused prompt with summary and user metadata.
